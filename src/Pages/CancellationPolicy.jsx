...
import { FiXCircle, FiSave, FiEdit2 } from "react-icons/fi";
...
const CancellationPolicy = () => {
  ...
  const fetchPolicy = async () => {
    ...
    try {
      const res = await fetch(
        `${baseUrl}/api/cancellation-policy?category=${encodeURIComponent(category)}`,
        {
          headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`${baseUrl}/api/cancellation-policy`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className="p-6 md:p-10">
      <header className="mb-8 bg-gradient-to-r from-orange-50 via-white to-orange-50 rounded-2xl p-6 shadow-sm border border-orange-200">
        <h1 className="text-3xl font-bold text-orange-900">Cancellation Policy Management</h1>
        <p className="text-orange-600 mt-1">
          Select a category to view, edit, and manage the cancellation policy.
        </p>
      </header>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <h2 className="text-xl font-semibold flex items-center gap-2 text-orange-900">
              <FiXCircle className="text-orange-600" /> Cancellation Policy
            </h2>
            <p className="text-orange-600">
              Enter the cancellation policy for the selected category.
            </p>
          </div>
