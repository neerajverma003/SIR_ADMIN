...
const Terms = () => {
  ...
  const fetchTerms = async () => {
    ...
    try {
      const res = await fetch(`${baseUrl}/api/terms?category=${encodeURIComponent(category)}`, {
        headers: {
  ...
  return (
    <div className="p-6 md:p-10">
      <header className="mb-8 bg-gradient-to-r from-orange-50 via-white to-orange-50 rounded-2xl p-6 shadow-sm border border-orange-200">
        <h1 className="text-3xl font-bold text-orange-900">Terms & Conditions</h1>
        <p className="text-orange-600 mt-1">
          Select a category to view, edit, and manage its terms and conditions.
        </p>
      </header>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-3 text-sm text-slate-600">
              Terms are managed per category. Select a category to edit its terms.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <h2 className="text-xl font-semibold flex items-center gap-2 text-orange-900">
              <FiFileText className="text-orange-600" /> Associated Terms
            </h2>
            <p className="text-orange-600">Enter the rules and terms for the selected category.</p>
          </div>
        ...
        <div className="mt-4">
          {loading ? (
            ...
            <textarea
              ...
              readOnly={!editing}
              placeholder="No terms defined yet. Click Edit to add terms."
